...
func TestDiagramToSceneGoldenChainSample(t *testing.T) {
	doc, err := ParseFile(filepath.Join("testdata", "diagrams", "chain_sample.poml"))
	if err != nil {
		t.Fatalf("parse diagram: %v", err)
	}
	scenes, err := diagramsToScenes(doc.Diagrams, defaultSceneExportOptions)
	...
	goldenPath := filepath.Join("testdata", "diagrams", "chain_sample_scene_sorted.json")
	wantBytes, err := os.ReadFile(goldenPath)
	if err != nil {
		t.Fatalf("read golden: %v", err)
	}
...
func TestDiagramToSceneGoldenBaseRoundtrip(t *testing.T) {
	doc, err := ParseFile(filepath.Join("testdata", "diagrams", "base_roundtrip.poml"))
	if err != nil {
		t.Fatalf("parse diagram: %v", err)
	}
	scenes, err := diagramsToScenes(doc.Diagrams, defaultSceneExportOptions)
	...
	goldenPath := filepath.Join("testdata", "diagrams", "base_roundtrip_scene_sorted.json")
	wantBytes, err := os.ReadFile(goldenPath)
	if err != nil {
		t.Fatalf("read golden: %v", err)
	}
