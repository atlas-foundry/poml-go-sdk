...
- ✅ Tracing parity (80%): OTEL exporters + deterministic `--trace-seed` landed; span attributes mirror parse/validate/convert; seeded trace goldens ship for parse/validate/convert with deterministic span ordering.
...
- ✅ Ecosystem bridges (80%): OpenAI/LangChain converters solid; CLI ergonomics improved (config/allowlists). Experimental LangGraph helpers live in `internal/langgraph`.
...
- [ ] Continue expanding trace scenario fixtures for additional CLI workflows.
