...
- Extended mode: opt-in mixed text + POML, `<op>`/`<figure>`, `<text>` blocks, unknown passthrough; root opt-in via `mode="extended"` or CLI flags.
- Diagram/scene round-trips with attrs/extras; `base_document` seeding keeps meta/role/tasks when rehydrating POML.
- CLI: `poml mcp` serves AST/validation/convert endpoints with optional tracing (stdout/OTLP).
...
Extended mode lets you mix plain text and POML (`<poml mode="extended"> ...`). Keep it off unless you need mixed content.
...
```
[ POML file ] --parse/validate--> [ poml-go SDK ] --convert--> [ Chat JSON / tools / scenes ]
         ↑                               ↑                          ↑
         │                               │                          │
     tests/goldens                 builder / CLI (mcp)          converters/output adapters
```
```
...
- Root opt-in: set `<poml mode="extended">` to enable Extended parsing/validation for a document. Otherwise Extended stays off unless you pass the CLI flags below.
