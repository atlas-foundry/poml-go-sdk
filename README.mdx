...
### `pomp` CLI (MCP starter)

We ship an experimental Go CLI, `pomp`, with a `poml mcp` subcommand that serves a minimal MCP-style HTTP surface for AST inspection:

```bash
go run ./cmd/pomp poml mcp --file hello.poml --addr :7777
```
