...
- Extended mode: opt-in mixed text + POML (`mode="extended"`), `<op>/<figure>/<text>` blocks, unknown passthrough; strict validation enforces names/JSON args/alt/mime/size/attrs; aliases via build tags; optional embedded-tag extraction flag; formatting tags handled as structured segments.
- Diagram/scene round-trips with attrs/extras; `base_document` seeding keeps meta/role/tasks when rehydrating POML.
- CLI: `poml mcp` serves AST/validation/convert endpoints with optional tracing (stdout/OTLP) and configurable allowlists via flags/env/POML config; formatting/meta attrs surfaced in converters.
...
- 1️⃣ Core fidelity polish (Δ→ 100%): Meta/Components covered (formatting tags as structured segments, meta min/max/components/stylesheet surfaced), lossless round-trips stable, tool wiring validated. Ongoing: keep goldens in sync with upstream.
- 2️⃣ Tracing parity (Δ→ 30%): tracing hooks planned; Go-side spans/fixtures pending.
- 3️⃣ Extended runway (Δ→ 70%): Extended tags (`<op>/<figure>/<text>`), strict validation (names/JSON args/alt/mime/size/attrs), allowlists + aliases/build-tags, embedded-tag extraction flag, mixed/media/text-escape goldens. Remaining: upstream proposal deltas only.
- 4️⃣ Ecosystem bridges (Δ→ 60%): OpenAI/LangChain converters solid; CLI ergonomics improved (config/allowlists).
...
ASCII doc flow:
```
Specs & code ▶ tests/goldens ▶ docs (POML + README) ▶ demos ▶ users/agents
```
