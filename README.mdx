...
- 1️⃣ Core fidelity polish (Δ→ 95%): core Meta/Components largely covered; lossless round-trips stable; remaining edge cases and strict Extended goldens under active work.
- 2️⃣ Tracing parity (Δ→ 30%): tracing hooks planned; Go-side spans/fixtures pending.
- 3️⃣ Extended runway (Δ→ 70%): Extended tags (`<op>/<figure>/<text>`), strict validation (names/JSON args/alt/mime/size/attrs), allowlists + aliases/build-tags, embedded-tag extraction flag, mixed/media/text-escape goldens. Remaining: proposal-specific attr enums/goldens if upstream adds more.
