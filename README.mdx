...
- Extended mode: opt-in mixed text + POML (`mode="extended"`), `<op>/<figure>/<text>` blocks, unknown passthrough; strict validation enforces names/JSON args/alt/mime/size/attrs; aliases via build tags; optional embedded-tag extraction flag; formatting tags handled as structured segments; MIME/op-kind allowlists configurable via flags/env/POML (defaults cover png/jpeg/webp/svg/tiff/heic/avif + flac/ogg/opus/aac/mp4 + mp4/webm/quicktime/mpeg/matroska/avi/m4v/3gpp).
...
- 3️⃣ Extended runway (Δ→ 78%): Extended tags (`<op>/<figure>/<text>`), strict validation (names/JSON args/alt/mime/size/attrs), richer allowlists + aliases/build-tags, embedded-tag extraction flag, mixed/media/text-escape goldens. Remaining: proposal deltas (custom data/media blocks, stricter enums) — see plan below.
