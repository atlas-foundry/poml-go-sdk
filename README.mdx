...
- CLI: `poml mcp` serves AST/validation/convert endpoints with optional tracing (stdout/OTLP/seeded in-memory), `--trace-seed` for deterministic spans (goldens via `poml.NewTraceRecorder`), and configurable allowlists via flags/env/POML config; formatting/meta attrs surfaced in converters.
