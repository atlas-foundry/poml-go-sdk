...
- 1️⃣ Core fidelity polish (Δ→ 98%): Meta/Components covered (formatting tags as structured segments, meta min/max/components/stylesheet surfaced), lossless round-trips stable, tool wiring validated. Remaining edge cases tracked in `meta.plan.poml`.
- 2️⃣ Tracing parity (Δ→ 60%): OTEL exporters + deterministic `--trace-seed` landed; span attributes mirror parse/validate/convert; seeded trace goldens ship for parse/validate/convert. Pending: CLI helpers to dump spans on demand and more scenario fixtures.
- 3️⃣ Extended runway (Δ→ 100%): Extended tags (`<op>/<figure>/<text>/<data>`), strict validation (names/JSON args/alt/mime/size/attrs), richer allowlists + aliases/build-tags, embedded-tag extraction flag, mixed/media/text-escape/data goldens. Remaining work focuses on keeping fixtures/allowlists aligned with new proposal tweaks — see plan below.
- 4️⃣ Ecosystem bridges (Δ→ 60%): OpenAI/LangChain converters solid; CLI ergonomics improved (config/allowlists). Experimental LangGraph helpers live in `internal/langgraph` — optional, not required for core flow.

### Extended completion plan (proposal-aligned)
- Finalize attribute enums + validation per proposal (media kinds/MIME, op kinds, object syntax).
- Add remaining Extended element cases (custom media/data blocks) with lossless goldens and converter coverage.
- Strengthen mixed-text segmentation + escape handling; expand round-trip fixtures for media and text.
- Document runtime knobs (allowlists, embedded-tag extraction, build tags for aliases) with copy/paste examples.
- Keep `ExtendedOff` compatibility goldens to guard default builds.
...
- [ ] Ship trace support in Go mirroring Python’s `trace/` semantics, with golden test fixtures and CLI flag parity (seeded spans already wired; initial parse/validate/convert fixture landed).
- [ ] Add converter round-trip goldens for extended tags (hint/example/object/data) and publish % coverage alongside CI; keep Extended-off compatibility fixtures; track expanded media/data enums.
...
- [ ] Expand Extended subset beyond `<op>/<figure>` (e.g., richer media/data tags) behind opt-in flags; publish allowlist knobs in CLI/config.
