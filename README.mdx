...
- CLI quickstart: parse → validate → convert a small POML file with `poml mcp`; inspect `/ast`, `/validate`, and `/convert` outputs.
- Go builder: construct a `poml.Document` via fluent APIs, render back to POML, and convert to OpenAI Chat JSON; see snippets above and fixtures under `poml/testdata/examples`.
