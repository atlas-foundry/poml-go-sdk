...
Production-ready Go library for **Prompt Orchestration Markup Language (POML)** with lossless parsing, validation, rendering, and converters for OpenAI Chat, LangChain, scene/diagram formats, and more. Built to keep prompt markup deterministic, multi-modal, and ready for agents, CLIs, and IDE extensions.

> **v0.0.8** — Aligned with [poml-lang/poml](https://github.com/poml-lang/poml) upstream. 80%+ test coverage, comprehensive fixture suite, zero known vulnerabilities.
...
- Extended mode: opt-in mixed text + POML (`mode="extended"`), `<op>/<figure>/<text>/<data>` blocks, unknown passthrough; strict validation enforces names/JSON args/alt/mime/size/attrs; aliases via build tags; optional embedded-tag extraction flag; formatting tags handled as structured segments; MIME/op-kind allowlists configurable via flags/env/POML (defaults cover png/jpeg/webp/svg/tiff/heic/avif + flac/ogg/opus/aac/mp4 + mp4/webm/quicktime/mpeg/matroska/avi/m4v/3gpp + json/xml/octet-stream for data/objects).
- Diagram/scene round-trips with attrs/extras; `base_document` seeding keeps meta/role/tasks when rehydrating POML.
- CLI: `poml mcp` serves AST/validation/convert endpoints with optional tracing (stdout/OTLP/seeded in-memory), `--trace-seed` for deterministic spans (goldens via `poml.NewTraceRecorder`), `/traces` dumps recorded spans when seeded, and configurable allowlists via flags/env/POML config; formatting/meta attrs surfaced in converters.
...
- Parity checklist & CI gates: `docs/parity_checklist.poml`
- Agent rules: `AGENTS.poml`
- Go quality spec: `go.quality.poml`
- Runnable examples: `examples/` (see below)
- Test fixtures: `poml/testdata/examples/*`

**Examples**
| Example | Description |
|---------|-------------|
| [`examples/01_basic_conversion`](examples/01_basic_conversion) | Parse, validate, convert to multiple formats |
| [`examples/02_format_comparison`](examples/02_format_comparison) | Side-by-side comparison of all output formats |
| [`examples/03_tool_calls`](examples/03_tool_calls) | Tool definitions, requests, and responses |
| [`examples/04_multimedia`](examples/04_multimedia) | Images, audio, and video handling |
| [`examples/05_templates`](examples/05_templates) | Variables, conditionals, loops, includes |
| [`examples/06_integrations`](examples/06_integrations) | MLflow, AgentOps, Weave, OpenTelemetry |

**Integration Guides**
- Converter formats: [`docs/converters.md`](docs/converters.md)
- Observability plugins: [`integrations/README.md`](integrations/README.md)
...
- **Core spec (meta + components)**: Implemented with strict parsing/validation and round-trip rendering. See the canonical docs: [Meta](https://microsoft.github.io/poml/latest/language/meta/) and [Components](https://microsoft.github.io/poml/latest/language/components/). Coverage: 100% of the published core; any regressions get tracked in `README.poml` and `meta.plan.poml`.
...
- **Tracing**: Python trace semantics are a reference point ([Trace docs](https://microsoft.github.io/poml/latest/python/trace/)). Go tracing now ships span emitters plus OTLP/stdout/seeded providers; CLI span dumps and additional fixtures remain (roughly 60% parity).
...
- ✅ Core fidelity (100%): Meta/Components fully covered (formatting tags as structured segments, meta min/max/components/stylesheet surfaced), lossless round-trips stable, tool wiring validated. Comprehensive fixture suite covering templates, rich text, data components, stylesheets, and version constraints.
- ✅ Tracing parity (100%): Full OpenTelemetry tracing with hierarchical child spans for all operations:
  - Root spans: `poml.parse`, `poml.validate`, `poml.convert`
  - Format spans: `poml.convert.message_dict`, `poml.convert.openai_chat`, `poml.convert.langchain`, etc.
  - Template spans: `poml.template.expand`, `poml.template.let`, `poml.template.interpolate`, `poml.template.include`
  - Processing spans: `poml.stylesheet.apply`, `poml.token.enforce`, `poml.component.filter`
  - Error categorization via `poml.error.type` attribute
- ✅ Extended runway (100%): Extended tags (`<op>/<figure>/<text>/<data>`), strict validation (names/JSON args/alt/mime/size/attrs), richer allowlists + aliases/build-tags, embedded-tag extraction flag, mixed/media/text-escape/data goldens all implemented.
- ✅ Ecosystem bridges (100%): OpenAI/LangChain/Pydantic converters solid; CLI ergonomics improved (config/allowlists). Experimental LangGraph helpers live in `internal/langgraph`.

### Test Coverage

The SDK maintains **80%+ test coverage** across all packages, enforced by CI. Test fixtures cover:
- Core POML parsing and validation
- Template directives (variables, let, conditionals, loops, includes)
- Rich text formatting (inline, blocks, lists, code)
- Data components (tables, folder trees, web extracts, conversations)
- Stylesheets and token limits
- Version constraint enforcement
- Extended mode validation (strict/lenient)

## ✅ Maintenance TODOs

- [x] Add richer demos across converters and docs for end-to-end journeys. *(See `examples/` directory)*
- [x] Keep fixtures aligned with upstream spec updates. *(Synced with microsoft/poml v0.0.8)*
...
- Config via POML: use `--config-poml poml/testdata/examples/validation_config.poml` to pull allowlists from `<object name="allowed-mime">[...]</object>` and `<object name="allowed-op-kinds">[...]</object>`.
  ...
  - Formatting/meta: formatting tags are emitted as structured `format` segments in converters; meta min/max/components/stylesheet fields surface in dict/OpenAI outputs.
...
* `/traces` — deterministic span dump (enable with `--trace-seed`, or by wiring your own `poml.NewTraceRecorder`)
...
* Future docs + demos when feature set stabilizes.
...
- **Content set**: `README.mdx` (landing), `README.poml` (structured), `PLAN.poml` / `meta.plan.poml` (execution), `go.quality.poml` (code bar), more guides in `docs/*.poml`.
- **Quality bar**: every feature ships with docs + runnable example; linkcheck/spellcheck + gofmt for snippets; coverage/spec % published.

ASCII doc flow:
```
Specs & code ▶ tests/goldens ▶ docs (POML + README) ▶ users/agents
```
...
- Quick compliance matrix (core + extended fixtures, strict defaults): `go test ./poml -run Compliance -count=1 -v | grep "Compliance matrix"`
- Full suite: `go test ./...`
